...
# form phase file
output = open('./NLL/obs/phase.obs', 'w+')
...
for line in Lines:
    ...
    if line[0] == ' ':
        ...
        for i in range(0, length//12):
            ...
            output.write(f'{sta}  ?  ?  ?  {phase}  ?  {time.strftime("%Y%m%d")}  {time.strftime("%H%M")}  {time.strftime("%S.%f")} GAU  2.00e-02 -1.00e+00 -1.00e+00 -1.00e+00\n')

output.close()
# form station.dat
f = open('./NLL/obs/station_coordinates.txt', 'w+')
