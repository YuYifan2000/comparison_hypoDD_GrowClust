...
# set up sources
np.random.seed(rank)
x = 30 + (np.random.rand(source_num,1)-0.5) * 30
y = 30 + 0.5* (x-30)
z = 12 + 0.4 * (x-30) + 0.4 * (y-30)
print('depth',z)
clust = np.ones([source_num,1])
sources = np.hstack([x*dx,y*dy,z*dz, clust])
...
for source in sources:
    ...
    o_sources.append([lat,lon, source[2], source[3]])
o_sources = np.array(o_sources)
...
recv = None
if rank == 0:
    recv = np.empty([size, source_num, 4])
comm.Gather(o_sources, recv, root=0)
if rank == 0:
    np.save('station', np.array(o_stations))
    np.save('source', np.vstack(recv))
    #plot_source_station(np.array(o_stations), np.vstack(recv))
...
for i in range(0, len(o_sources)):
    ...
    tp = fmm.eikonal(p_vel_structure,xyz=sources[i][:3],ax=[0,dx,nx],ay=[0,dy,ny],az=[0,dz,nz],order=2).reshape(nx,ny,nz,order='F')
    tp = tp[:,:,0]
    ts = fmm.eikonal(s_vel_structure,xyz=sources[i][:3],ax=[0,dx,nx],ay=[0,dy,ny],az=[0,dz,nz],order=2).reshape(nx,ny,nz,order='F')
    ts = ts[:,:,0]
    for j in range(0, len(stations)):
        p_time_table[i][j] = tp[stations[j][0], stations[j][1]]
        s_time_table[i][j] = ts[stations[j][0]][stations[j][1]]
...
recv_s = None
if rank == 0:
    recv_p = np.empty([size, len(o_sources), len(o_stations)])
    recv_s = np.empty([size, len(o_sources), len(o_stations)])
