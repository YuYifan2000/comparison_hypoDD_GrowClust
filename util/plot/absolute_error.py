...
# for xcorloc
catalog_xcloc = pd.read_csv(f"./data/2.loc_xcor", sep="\s+", names=["qID", "yr","mon","day", "hr","min","sec","lat","lon","dep","mag","pair", "pick_used", "erx", "ery", "erz", "ert", "rms_phs", "rms_dif", "type"])

# for hyposvi
catalog_hyposvi = pd.read_csv(f"./data/catalog_svi9.csv_svi")
...
for i in range(0, len(sources)):
    lat1 = catalog_hyposvi['latitude'][i]
    lon1 = catalog_hyposvi['longitude'][i]
    lat0 = sources[i,0]
    lon0 = sources[i,1]
    dist, az, baz = gps2dist_azimuth(lat0, lon0, lat1, lon1)
