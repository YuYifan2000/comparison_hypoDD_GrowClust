...
np.random.seed(rank)
x = 300 + (np.random.rand(source_num,1) - 0.5) * 30
y = 300 - (np.random.rand(source_num,1) - 0.5) * 40
...
recv = None
if rank == 0:
    recv = np.empty([size, source_num, 3])
...
f = open(f'./tmp/{rank}test.pha', 'w+')
f2 = open(f'./tmp/{rank}benchmark_station.txt', 'w')
...
if rank == 0:
    np.save('./hypoDD/tt_P',np.vstack(recv_p))
    np.save('./hypoDD/tt_S',np.vstack(recv_s))
    f = open('./hypoDD/test.pha', 'w+')
    for i in range(0, size):
        f1 = open(f'./tmp/{i}test.pha', 'r')
        Lines = f1.readlines()
        f1.close()
        for line in Lines:
            f.write(line)
    f.close()
    f = open('./hypoDD/benchmark_station.txt', 'w')
    for i in range(0, size):
        f1 = open(f'./tmp/{i}benchmark_station.txt', 'r')
        Lines = f1.readlines()
        f1.close()
        for line in Lines:
            f.write(line)
