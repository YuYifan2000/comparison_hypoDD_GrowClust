...
#ax.scatter(sources[:,0]+(np.random.rand(n)-0.5)/5., sources[:,1]+(np.random.rand(n)-0.5)/5., sources[:,2]+(np.random.rand(n)-0.5)*1.2, zdir='z', label='random', s=2)
...
fig = plt.figure()
ax = fig.add_subplot()
ax.scatter(sources[:,0], sources[:,1], label='sources', marker = "*", c='blue', s=3.5)
ax.scatter(catalog_hypoDD['LAT'], catalog_hypoDD['LON'], label='hypoDD',  c='red', s=2.5)
ax.scatter(catalog_gc['latR'], catalog_gc['lonR'], label='GrowClust',  c='green', s=2.5)
ax.legend()
ax.set_xlabel('Latitude')
ax.set_ylabel('Longitude')
plt.show()